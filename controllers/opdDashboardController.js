...
const getOpdDashboardPage = async (req, res) => {
    try {
        ...
        if (appointments.length === 0) {
            return res.status(200).render('opdDashboard', {
                mesg2: true
            });
        } else {
            return res.status(200).render('opdDashboard', {
...
const getAppDetail = async (req, res) => {
    try {
        ...
        return res.status(200).render('appointmentDetails', {
            mesg1: doctor,
            mesg2: appointments
        });
    } catch (e) {
...
const deleteAppointments = async (req, res) => {
    try {
        ...
        const doctor = await Doctor.findOne({
            where: {
                D_ID: patient.Patient_Appointment_Details[0].Patient_Appointments.Doctor_ID
            },
            attributes: ['D_ID'],
            include: [{
                model: User,
...
const getConfirmedAppointmentsPage = async (req, res) => {
    try {
        ...
        if (newAppointments.length === 0 && reAppointments.length === 0) {
            return res.status(200).render('confirmedAppointmentsOPD', {
                mesg2: true
            });
        } else {
            return res.status(200).render('confirmedAppointmentsOPD', {
                mesg1: newAppointments,
                mesg3: reAppointments
            });
        }
...
const makeOpdCard = async (req, res) => {
    try {
        ...
        if (opdCardExists.length === 0) {
            ...
            let cardNo = await HealthLog.create(opdCard).then(result => {
            ...
            return res.status(200).render('opdCard', {
                mesg1: patient,
                mesg2: doctor,
                mesg3: appointments,
                mesg4: patientOPDcard
            });
        } else {
            return res.status(200).render('opdCard', {
                mesg5: true
...
const getTodaysLabTests = async (req, res) => {
    try {
        ...
        if (patient.length === 0) {
            return res.status(200).render('labReports', {
                mesg1: true
            });
        } else {
            return res.status(200).render('labReports', {
...
const getopdCardSearchPage = async (req, res) => {
    try {
        ...
        return res.status(200).render('OPDCardSearchOPD', {
            opdsearchpage: true
        });
...
const getIndividualOpdCardDetails = async (req, res) => {
    try {
        ...
        return res.status(200).render('OPDcardDetailsOPD', {
            mesg1: patient,
            mesg2: doctor,
            opdsearchmesg4: getopdCard,
            mesg5: cardNumber
        });
