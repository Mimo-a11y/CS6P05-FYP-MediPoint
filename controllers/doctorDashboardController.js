...
const getTodaysOPDcard = async (req, res) => {
    try {
        ...
        if (opdCard.length === 0) {
            return res.status(200).render('incomingVisit', {
                mesg2: true
            });
        } else {
            return res.status(200).render('incomingVisit', {
...
const getPatientOpdCard = async (req, res) => {
    try {
        ...
        cardNumber[0] = {
            ...
            D_ID: req.params.did,
            P_ID: req.params.pid,
            App_Type: appointments.Patient_Appointment_Details[0].App_Type
        };
        return res.status(200).render('OPDcardDetailsIncomingVisit', {
            mesg1: patient,
            mesg2: doctor,
            mesg4: getopdCard,
            mesg3: appointments,
...
const getVisitDetails = async (req, res) => {
    try {
        ...
        if (healthLog.BP === null && healthLog.Pulse === null && healthLog.Temperature === null && healthLog.Symptoms_Exp === null && healthLog.Diagnosis === null && healthLog.LabReportReportID === null && healthLog.PrescriptionPresID === null) {
            return res.render('patientVisitDetailsIncomingVisit', {
                mesg1: healthLog
            });
        } else {
            return res.render('patientVisitDetailsIncomingVisit', {
...
const updateVisitDetails = async (req, res) => {
    try {
        ...
        let reportID = await LabReports.create(labData).then(result => {
        ...
        if (req.body.medName instanceof Array) {
            ...
            let presID = await Prescriptions.create(medicineData).then(result => {
...
const getopdCardSearchPageByDoctor = async (req, res) => {
    try {
        ...
        return res.status(200).render('OPDCardSearch', {
...
const getIndividualOpdCardDetailsByDoc = async (req, res) => {
    try {
        ...
        return res.status(200).render('OPDCardDetailsDoctor', {
            mesg1: patient,
            mesg2: doctor,
            docsearchmesg4: getopdCard,
            mesg5: cardNumber
        });
...
const getVisitDetailsByDoc = async (req, res) => {
    try {
        ...
        if (healthLog.BP === null && healthLog.Pulse === null && healthLog.Temperature === null && healthLog.Symptoms_Exp === null && healthLog.Diagnosis === null && healthLog.LabReportReportID === null && healthLog.PrescriptionPresID === null) {
            return res.render('patientVisitDetailsByDoc', {
                mesg8: true
            });
        } else {
            return res.render('patientVisitDetailsByDoc', {
